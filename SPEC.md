...
## กติกาการตั้งราคา (ตกลงกับ Pist 12 ก.ย. 2026)

**ใครดูแล:** Claude ดูแลราคาและช่วงเทศกาลทั้งหมดในโค้ด — Pist ไม่แก้เอง ไม่มีหน้า admin ให้แก้
ถ้าต้องเปลี่ยนราคาหรือเพิ่มช่วงเทศกาล Pist สั่ง Claude แก้ให้

**ที่มาของราคา:** ฐานข้อมูล `rooms` (`price` = ไม่รวมอาหารเช้า, `price_bf` = รวมอาหารเช้า, `extra_max` = เตียงเสริมสูงสุด)
ตั้งค่าครั้งแรกผ่าน migration ใน `worker.js` — แก้ราคาภายหลังต้องเขียน migration ใหม่ที่เช็คค่าเดิมก่อนทับ

**คิดราคาทีละคืน** ไม่ใช่ราคาคูณจำนวนคืน เพราะแต่ละคืนอาจคนละเรต (ดู `priceStay`)
"คืน" หมายถึงวันเช็คอินของคืนนั้น

**ตัวคูณ**
1. ศุกร์และเสาร์ +20% (`WEEKEND_UP`)
2. ช่วงเทศกาลตาม `SEASONS` — `from`/`to` รวมปลายทั้งสองข้าง
3. **ไม่ทบกัน** ใช้ตัวที่สูงกว่าตัวเดียว — คืนเสาร์ในช่วงปีใหม่ได้ +40% ไม่ใช่ +68%
4. เตียงเสริมคิดราคาคงที่ต่อคืน ไม่ปรับตามวันหรือเทศกาล

**ห้ามให้ลูกค้าเห็นว่ามีการปรับราคา** (Pist ย้ำหลายครั้ง)
- กล่องสรุปยอดห้ามโชว์ราคาฐานคู่กับยอดจริง ต้องเป็นตัวเลขที่บวกกันลงตัวเท่านั้น
- หน้าห้องเขียน "ราคาเริ่มต้นต่อคืน" + "อาจปรับตามช่วงวันหยุดและเทศกาล" เท่านั้น ห้ามระบุว่าวันไหนขึ้นเท่าไหร่
- ห้ามมีบรรทัดแยกประเภท "ค่าธรรมเนียมวันหยุด" ที่ไหนทั้งสิ้น

**ราคาที่จองไปแล้วไม่เปลี่ยนย้อนหลัง** — `bookings.amount` ล็อกยอดไว้ตอนจอง ต้องตรงกับเงินที่ลูกค้าโอนจริงเสมอ

**เมื่อแก้ราคาบนเว็บ** ต้องแก้ให้ครบทุกจุด: กล่องราคาหน้าห้อง · หน้าแรก · meta description · เลขกีบ (×650) · dict ใน `build_i18n.py` ทั้ง EN และ LO
แล้วไล่ตรวจทีละห้องเทียบกับฐานข้อมูล — **ห้ามใช้ replace ข้อความเหวี่ยงแหทั้งเว็บ** เพราะราคาซ้ำกันข้ามห้องได้ (เคยทำหน้าเจ้าสัวพังมาแล้ว)
...
- **หน้า detail เต็นท์ (20 ส.ค. 2026):** /rooms/tent/ ครบ 3 ภาษา — T1-T8 ทั้ง 8 ใน
  ...
  tent-lawn-view / **หน้า detail ครบทั้ง 9 เฮือน + เต็นท์ แล้ว 23 ส.ค. 2026**
- **หน้า detail เฮือนมหาเศรษฐี R1 (23 ส.ค. 2026):** /rooms/mahasetthi/ ครบ 3 ภาษา
  ...
  mahasetthi-bed-2 + mahasetthi-bed-3 (ห้องผนังไม้ไผ่สาน 2 ห้อง — R1 มี 3 ห้องนอนครบแล้ว),
  mahasetthi-garden (สนามหญ้า/ไฮเดรนเยีย/ทางเดินหิน)
...
- **หน้า detail เฮือนล้ำลวย R6 (23 ส.ค. 2026):** /rooms/lamluay/ ครบ 3 ภาษา
  — **slug = lamluay** ตามชื่อป้าย/NAME_MAP (Lam Luay) แต่รูปการ์ดหน้าแรกยังชื่อเก่า
  room-ramruay.webp (ไม่เปลี่ยน กัน cache)
  ✅ **ชื่อยืนยันแล้ว 23 ส.ค. 2026:** ป้ายหน้าบ้านเขียน "เฮือนล้ำลวย" — ปิดประเด็นสะกด
  (สมุดจองเก่าเขียน "เรือนร่ำรวย" เป็นชื่อเก่า ใช้เฉพาะใน tools/build_import.py map เข้า R6)
- **หน้า detail เฮือนมั่งมีเงินทอง R5 (23 ส.ค. 2026) — ปิดครบทั้ง 9 เฮือน:**
  /rooms/mangmee/ ครบ 3 ภาษา · pill ไม่ระบุจำนวนห้องนอน/ห้องน้ำ (การ์ดหน้าแรกไม่มีข้อมูล
  ห้ามเดา) · รูป 2 ใบ: mangmee-hero + mangmee-bed-2
  ⚠ **ต้นฉบับกว้างแค่ 1320px** (ไม่ใช่ไฟล์กล้อง) — ไม่ upscale ตามกฎ 4 hero เลยเป็น
  1320x743 ไม่ใช่ 1920x1080 · ถ้าได้ไฟล์เต็มจากกล้องให้ gen ใหม่ทับ (rename -v2)
  ⚠ รูปเห็น **เตียง 2 หลัง** แต่การ์ดเขียน "พักได้ 2 ท่าน" — รอ Pist ยืนยันว่าขายกี่ท่าน
  ⚠ ราคา 800/1,000 ยังเป็นข้อค้างเดิมในหมวด 7 ข้อ 2 (รอ Pist ยืนยัน label อาหารเช้า)
  รูปครบ 23 ส.ค. 2026: lamluay-hero-v2 (ระเบียงหน้าบ้าน — เดิม hero เป็นห้องนอน
  เปลี่ยนแล้ว rename เป็น -v2 ตามกฎ 4), lamluay-bed, lamluay-bath + lamluay-bath-2
  (ห้องน้ำแนวตั้ง เก็บสัดส่วนเดิม ไม่ครอป) / ยังไม่มีคลิป
  📌 **บทเรียนอัปโหลด:** iPhone ที่แก้รูปในแอป Photos จะตั้งชื่อ FullSizeRender.jpeg
  เหมือนกันทุกใบ — ส่งพร้อมกันหลายใบจะทับกันเหลือใบเดียว ให้ส่งทีละใบ
  และเช็ค md5sum/ขนาดไฟล์ก่อนใช้เสมอว่าซ้ำกับที่มีอยู่ไหม
  ✅ เคลียร์แล้ว 23 ส.ค. 2026: ห้องเตียงคู่ (โต๊ะเครื่องแป้ง+กระจกขาว+ผ้าคาดคราม)
  เป็นของ **มหาเศรษฐี** — รูปในชุดมหาเฮงคือมุมที่สองของห้องเดียวกัน ส่งปนมา
